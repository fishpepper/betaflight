...
static void osdConvertToAbsolutePosition(uint8_t item, int8_t *pos_x, int8_t *pos_y) {
    ...
    uint8_t origin = osdConfig()->item[item].origin;
    ...
    if (origin & OSD_ORIGIN_E) {
        // move east
        elemPosX += display_width / 2;
    }

    if (origin & OSD_ORIGIN_W) {
        // move west
        elemPosX -= display_width / 2;
    }

    if (origin & OSD_ORIGIN_N) {
        // move north
        elemPosY -= display_height / 2;
    }

    if (origin & OSD_ORIGIN_S) {
        // move south
        elemPosY += display_height / 2;
    }
...
{
    if (!osdConfig()->item[item].visible || BLINK(item)) {
        return;
    }
...
void pgResetFn_osdConfig(osdConfig_t *osdConfig)
{
    ...
    osdConfig->item[OSD_ITEM_TIMER_2]       = (osdItem_t) {   1,  1, OSD_ORIGIN_NW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_DEBUG]              = (osdItem_t) {   1,  0, OSD_ORIGIN_NW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_GPS_LAT]            = (osdItem_t) {   1,  2, OSD_ORIGIN_NW, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_RSSI_VALUE]         = (osdItem_t) {  -6,  1, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_MAIN_BATT_VOLTAGE]  = (osdItem_t) {   2,  1, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_AVG_CELL_VOLTAGE]   = (osdItem_t) {   2,  2, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_DISARMED]           = (osdItem_t) {   4,  4, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_GPS_SATS]           = (osdItem_t) {   5,  1, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_GPS_LON]            = (osdItem_t) {   4,  2, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_ESC_TMP]            = (osdItem_t) {   4,  2, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_ESC_RPM]            = (osdItem_t) {   5,  2, OSD_ORIGIN_N, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_ITEM_TIMER_1]       = (osdItem_t) {  -7,  1, OSD_ORIGIN_NE, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_GPS_SPEED]          = (osdItem_t) {  -3, -1, OSD_ORIGIN_E, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_ALTITUDE]           = (osdItem_t) {  -6,  0, OSD_ORIGIN_E, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_NUMERICAL_HEADING]  = (osdItem_t) {  -6,  2, OSD_ORIGIN_E, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_NUMERICAL_VARIO]    = (osdItem_t) {  -6,  1, OSD_ORIGIN_E, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_VTX_CHANNEL]        = (osdItem_t) {  -4, -4, OSD_ORIGIN_SE, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_PIDRATE_PROFILE]    = (osdItem_t) {  -4, -5, OSD_ORIGIN_SE, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_FLYMODE]            = (osdItem_t) {  -1, -5, OSD_ORIGIN_S, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_CRAFT_NAME]         = (osdItem_t) {  -4, -4, OSD_ORIGIN_S, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_POWER]              = (osdItem_t) {   1, -5, OSD_ORIGIN_S, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_MAIN_BATT_USAGE]    = (osdItem_t) {  -6, -3, OSD_ORIGIN_S, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_CURRENT_DRAW]       = (osdItem_t) {   1,  0, OSD_ORIGIN_SW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_MAH_DRAWN]          = (osdItem_t) {   1, -1, OSD_ORIGIN_SW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_ROLL_PIDS]          = (osdItem_t) {   7,  -2, OSD_ORIGIN_SW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_PITCH_PIDS]         = (osdItem_t) {   7,  -1, OSD_ORIGIN_SW, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_YAW_PIDS]           = (osdItem_t) {   5,  -0, OSD_ORIGIN_SW, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_PITCH_ANGLE]        = (osdItem_t) {   1,  1, OSD_ORIGIN_W, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_ROLL_ANGLE]         = (osdItem_t) {   1,  2, OSD_ORIGIN_W, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_THROTTLE_POS]       = (osdItem_t) {   1,  0, OSD_ORIGIN_W, OSD_ITEM_VISIBLE};

    osdConfig->item[OSD_WARNINGS]           = (osdItem_t) {  -5,  3, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_HOME_DIST]          = (osdItem_t) {   1,  2, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_HOME_DIR]           = (osdItem_t) {   0,  2, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
    osdConfig->item[OSD_COMPASS_BAR]        = (osdItem_t) {  -4,  1, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};

    // Crosshair uses 3 chars, from center offset 1 to the left
    osdConfig->item[OSD_CROSSHAIRS]         = (osdItem_t) { -1,  0, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
    // AH top center of region is 4 to the left
    osdConfig->item[OSD_ARTIFICIAL_HORIZON] = (osdItem_t) {  0, -4, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
    // Horizon is centered
    osdConfig->item[OSD_HORIZON_SIDEBARS]   = (osdItem_t) {  0,  0, OSD_ORIGIN_C, OSD_ITEM_VISIBLE};
